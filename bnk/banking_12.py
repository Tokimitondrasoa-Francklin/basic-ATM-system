...
##Sergey Brin /Google
##Warren Buffet /Berkshine Hathaway
##Mukesh Ambani /diversfied
...
def register():
    ...
    Label(register_screen, text="PIN", font=('Calibri', 12)).grid(row=4,sticky=W)
    ...
    Entry(register_screen, textvariable=temp_pin).grid(row=4,column=0)
...
def finish_deposit():
    ...
    file.seek(0) #empty the file
    file.truncate(0) #update it
...
def withdraw():
    ...
    current_balance_label.grid(row=1, sticky=W)
    Label(withdraw_screen, text="Withdraw_Amount", font=('Calibri', 12)).grid(row=2, sticky=W)
...
def personal_details():
    ...
    details_PIN = user_details[4]
    details_balance = user_details[5]
    #Personal details screen
    personal_details_screen = Toplevel(master)
    personal_details_screen.title('Personal Details')
    #Labels
    Label(personal_details_screen, text='Personal Details', font=('Calibri', 14)).grid(row=0, sticky=N,pady=10)
    ...
    Label(personal_details_screen, text='PIN : '+details_PIN, font=('Calibri', 12)).grid(row=4, sticky=W)
...
def login():
    ...
    #Button
    Button(login_screen, text="Login", command=login_session, width =15, font=('Calibri', 12)).grid(row=3,sticky=W,padx=5)
...
#Image import
img = Image.open('../img/b01.png')
img = img.resize((200,200))
img = ImageTk.PhotoImage(img)

#Labels
Label(master, text = "Custom Banking Beta", font=('Calibri', 14)).grid(row=0, sticky=N,pady=10)
Label(master, text = "The most secure bank you've probably used in the world", font=('Calibri', 11)).grid(row=1, sticky=N)
Label(master, image=img).grid(row=2, sticky=N,pady=100)

#Buttons

Button(master, text="Register", font=('Calibri',14), width=20, command=register).grid(row=3, sticky=N)
Button(master, text="Login", font=('Calibri', 14), width=20, command=login).grid(row=4,sticky=N,pady=6)
